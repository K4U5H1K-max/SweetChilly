...
export default function DriverSafetyModal({ vehicleId, isOpen, onClose }) {
  ...
  // Mask driver phone for public privacy
  const maskedPhone = vehicle.driverPhone
    ? vehicle.driverPhone.replace(/^(\+?\d{2,3}-\d{2})\d+(\d{2,3})$/, '$1-***-$2')
    : '+91-98***-*****';
  ...
  // Trigger Mock Safety Call
  async function handleStartSafetyCheck() {
    setIsCalling(true);
    setErrorMessage('');
    setCallProgressState('QUEUED');

    // Smooth UI progress simulation to demonstrate lifecycle transitions
    ...
    try {
      ...
      // Allow visual progression to be seen
      setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden font-sans my-6 animate-fadeIn">
        ...
        {/* ========================================================
            MAIN BODY: SIMULATION CONTROLS & CALL SESSION
        ======================================================== */}
                ...
                {[
                  ...
                  return (
                    <div key={step.key} className="flex flex-col items-center gap-1">
                      ...
                      <span className={isCurrent ? 'text-blue-700 font-bold' : isPast ? 'text-slate-800' : 'text-slate-400'}>
                        {step.label}
                      </span>
                    </div>
                  );
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs font-bold text-white">
                    {callProgressState === 'IDLE' && 'Ready for Safety Session'}
                    ...
                    {callProgressState === 'COMPLETED' && 'Voice Session Concluded & Classified'}
                  </div>
              ...
              <div className="flex-1 space-y-3 overflow-y-auto max-h-[220px] pr-1">
                {activeSession?.dialogueHistory && activeSession.dialogueHistory.length > 0 ? (
                  activeSession.dialogueHistory.map((d, i) => {
                    const isAgent = d.speaker === 'AI Safety Agent';
                    return (
                      <div
                        key={i}
                  ...
                  <div className="h-28 flex flex-col items-center justify-center text-slate-400 text-xs">
                    ...
                    <span>No active transcript. Trigger a safety check to start voice simulation.</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {activeSession && (
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs space-y-3">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                  <div>
                    <span className="text-[10px] font-bold uppercase text-slate-400 block">Structured Safety Outcome</span>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase border ${getSafetyBadge(activeSession.structuredOutcome)}`}>
                        {activeSession.structuredOutcome}
                      </span>
                      <span className="text-[11px] text-slate-500 font-mono">
                        Confidence: {(activeSession.outcomeConfidence * 100).toFixed(0)}%
                      </span>
                    </div>
                  </div>

                  <div className="text-right">
                    <span className="text-[10px] font-bold uppercase text-slate-400 block">Escalation State</span>
                ...
                <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100 text-xs text-slate-700">
                  <span className="font-bold text-slate-900">Summary:</span> {activeSession.summary}
                </div>
