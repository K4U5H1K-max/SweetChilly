...
export default function StatusChip({
  ...
  if (['success', 'nominal', 'safe', 'available', 'depot ready', 'verified', 'on route'].includes(normalizedVariant)) {
    colorStyles = solid
      ? 'bg-[#16A34A] text-white border-[#16A34A]'
      : 'bg-emerald-50 text-emerald-700 border-emerald-200';
  } else if (['warning', 'caution', 'delayed', 'needs attention'].includes(normalizedVariant)) {
    colorStyles = solid
      ? 'bg-[#F59E0B] text-white border-[#F59E0B]'
      : 'bg-amber-50 text-amber-700 border-amber-200';
  } else if (['critical', 'blocked', 'flagged', 'emergency', 'danger'].includes(normalizedVariant)) {
    colorStyles = solid
      ? 'bg-[#DC2626] text-white border-[#DC2626] shadow-xs'
      : 'bg-rose-50 text-rose-700 border-rose-200';
  } else if (['info', 'active', 'in_transit', 'in transit', 'planned', 'dispatched'].includes(normalizedVariant)) {
    colorStyles = solid
      ? 'bg-[#2563EB] text-white border-[#2563EB]'
      : 'bg-blue-50 text-blue-700 border-blue-200';
  } else if (['ai', 'ai verified', 'ai analysis', 'smart'].includes(normalizedVariant)) {
    colorStyles = solid
