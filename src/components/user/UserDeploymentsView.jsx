      ...
      <div className="bg-white border border-slate-200/90 rounded-xl p-4 shadow-card">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3.5">
          ...
          <button
            ...
            className="px-4 py-2 bg-[#2563EB] hover:bg-blue-700 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-xs transition-all flex items-center gap-2 cursor-pointer shrink-0 touch-target"
          >
        ...
        <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl max-w-md">
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold transition-all touch-target sm:min-h-0 ${
          ...
          {ongoingList.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
              {ongoingList.map((dep) => (
                <div
                  key={dep.id}
                  className="bg-white border border-slate-200/90 hover:border-slate-300 rounded-2xl p-4 shadow-card hover:shadow-card-hover transition-all flex flex-col justify-between"
                >
                  {/* Card Header */}
                  <div>
                    <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-slate-100 mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-mono text-slate-500">{dep.id}</span>
                      </div>
                      <StatusChip
                  ...
                  <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-lg transition-colors cursor-pointer touch-target sm:min-h-0"
                      >
                        View Details
                      </button>
                      <button
                        onClick={() => handleReportFromDeployment(dep)}
                        className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-bold text-xs rounded-lg transition-colors cursor-pointer flex items-center gap-1.5 touch-target sm:min-h-0"
                        title="Report roadblock or hazard on this route"
                      >
                        <IconWarning className="w-3.5 h-3.5 text-rose-600" />
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-[#16A34A] hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-xs rounded-lg transition-colors cursor-pointer touch-target sm:min-h-0"
                    >
